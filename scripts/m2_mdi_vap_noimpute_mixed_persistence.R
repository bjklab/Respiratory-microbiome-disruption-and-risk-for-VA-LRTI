...
dat %>%
  ...
  mutate(shannon_flip_noimpute_count = as.character(cumsum(shannon_improved)),
  ) %>%
  tidyr::fill(shannon_flip_noimpute_count, .direction = "down") %>% #arrange(desc(shannon_flip_noimpute_count)) %>%
  group_by(subject_id, shannon_flip_noimpute_count) %>%
  arrange(subject_day) %>%
...
d_vap %>%
  brm(suspected_vap ~ shannon_low_count + 0 + (1|subject_id),
      ...
      backend = "cmdstanr",
      seed = 16) -> m_noimpute_sv_shannon_low_count_brms
m_noimpute_sv_shannon_low_count_brms

rstan::check_hmc_diagnostics(m_noimpute_sv_shannon_low_count_brms$fit)
...
m_noimpute_sv_shannon_low_count_brms %>%
  ...
  identity() -> m_noimpute_sv_shannon_low_count_param_est
m_noimpute_sv_shannon_low_count_param_est
}
...
#' mixed model plot
if(run_plots) {
m_noimpute_sv_shannon_low_count_brms$data %>%
  as_tibble() %>%
  expand(shannon_low_count = modelr::seq_range(shannon_low_count, n = 100), subject_id = unique(subject_id)) %>%
  tidybayes::add_fitted_draws(newdata = ., model = m_noimpute_sv_shannon_low_count_brms) %>%
  ungroup() %>%
...
d_vap %>%
  brm(suspected_vap_plus_signs_and_pathogen ~ shannon_low_count + 0 + (1|subject_id),
      ...
      control = list("adapt_delta" = 0.99, max_treedepth = 16),
      backend = "cmdstanr",
      seed = 16) -> m_noimpute_svp_noimpute_shannon_low_count_brms
m_noimpute_svp_noimpute_shannon_low_count_brms

rstan::check_hmc_diagnostics(m_noimpute_svp_noimpute_shannon_low_count_brms$fit)
...
m_noimpute_svp_noimpute_shannon_low_count_brms %>%
  ...
  as_tibble(rownames = "param") %>%
  mutate_if(is.numeric, list("OR" = ~ exp(.x))) %>%
  identity() -> m_noimpute_svp_noimpute_shannon_low_count_param_est
m_noimpute_svp_noimpute_shannon_low_count_param_est
}
...
#' mixed model plot
if(run_plots) {
m_noimpute_svp_noimpute_shannon_low_count_brms$data %>%
  as_tibble() %>%
  expand(shannon_low_count = modelr::seq_range(shannon_low_count, n = 100), subject_id = unique(subject_id)) %>%
  tidybayes::add_fitted_draws(newdata = ., model = m_noimpute_svp_noimpute_shannon_low_count_brms) %>%
  ungroup() %>%
  ggplot(data = ., aes(x = shannon_low_count, y = .value)) +
  tidybayes::stat_lineribbon(.width = c(0.5, 0.8, 0.95), alpha = 0.7, color = colorspace::sequential_hcl(5, palette = "Blues 3")[1]) +
  colorspace::scale_fill_discrete_sequential(palette = "Blues 3", nmax = 5, order = 2:4) +
...
d_vap %>%
  brm(suspected_vap_plus_abx_and_pathogen_and_signs ~ shannon_low_count + 0 + (1|subject_id),
      ...
      control = list("adapt_delta" = 0.99, max_treedepth = 16),
      backend = "cmdstanr",
      seed = 16) -> m_noimpute_svaps_shannon_low_count_brms
m_noimpute_svaps_shannon_low_count_brms

rstan::check_hmc_diagnostics(m_noimpute_svaps_shannon_low_count_brms$fit)
...
m_noimpute_svaps_shannon_low_count_brms %>%
  ...
  as_tibble(rownames = "param") %>%
  mutate_if(is.numeric, list("OR" = ~ exp(.x))) %>%
  identity() -> m_noimpute_svaps_shannon_low_count_param_est
m_noimpute_svaps_shannon_low_count_param_est
}
...
#' mixed model plot
if(run_plots) {
m_noimpute_svaps_shannon_low_count_brms$data %>%
  as_tibble() %>%
  expand(shannon_low_count = modelr::seq_range(shannon_low_count, n = 100), subject_id = unique(subject_id)) %>%
  tidybayes::add_fitted_draws(newdata = ., model = m_noimpute_svaps_shannon_low_count_brms) %>%
  ungroup() %>%
  ggplot(data = ., aes(x = shannon_low_count, y = .value)) +
  tidybayes::stat_lineribbon(.width = c(0.5, 0.8, 0.95), alpha = 0.7, color = colorspace::sequential_hcl(5, palette = "Blues 3")[1]) +
  colorspace::scale_fill_discrete_sequential(palette = "Blues 3", nmax = 5, order = 2:4) +
...
dat %>%
  ...
  mutate(log_copy_16S_per_ml_sputum_flip_noimpute_count = as.character(cumsum(log_copy_16S_per_ml_sputum_improved)),
  ) %>%
  tidyr::fill(log_copy_16S_per_ml_sputum_flip_noimpute_count, .direction = "down") %>% #arrange(desc(log_copy_16S_per_ml_sputum_flip_noimpute_count)) %>%
  group_by(subject_id, log_copy_16S_per_ml_sputum_flip_noimpute_count) %>%
  arrange(subject_day) %>%
...
d_vap %>%
  brm(suspected_vap ~ log_copy_16S_per_ml_sputum_high_count + 0 + (1|subject_id),
      ...
      control = list("adapt_delta" = 0.99, max_treedepth = 16),
      backend = "cmdstanr",
      seed = 16) -> m_noimpute_sv_log_copy_16S_per_ml_sputum_high_count_brms
m_noimpute_sv_log_copy_16S_per_ml_sputum_high_count_brms

rstan::check_hmc_diagnostics(m_noimpute_sv_log_copy_16S_per_ml_sputum_high_count_brms$fit)
...
m_noimpute_sv_log_copy_16S_per_ml_sputum_high_count_brms %>%
  ...
  as_tibble(rownames = "param") %>%
  mutate_if(is.numeric, list("OR" = ~ exp(.x))) %>%
  identity() -> m_noimpute_sv_log_copy_16S_per_ml_sputum_high_count_param_est
m_noimpute_sv_log_copy_16S_per_ml_sputum_high_count_param_est
}
...
#' mixed model plot
if(run_plots) {
m_noimpute_sv_log_copy_16S_per_ml_sputum_high_count_brms$data %>%
  as_tibble() %>%
  ...
  tidybayes::add_fitted_draws(newdata = ., model = m_noimpute_sv_log_copy_16S_per_ml_sputum_high_count_brms) %>%
  ungroup() %>%
...
d_vap %>%
  brm(suspected_vap_plus_signs_and_pathogen ~ log_copy_16S_per_ml_sputum_high_count + 0 + (1|subject_id),
      ...
      control = list("adapt_delta" = 0.99, max_treedepth = 16),
      backend = "cmdstanr",
      seed = 16) -> m_noimpute_svp_noimpute_log_copy_16S_per_ml_sputum_high_count_brms
m_noimpute_svp_noimpute_log_copy_16S_per_ml_sputum_high_count_brms

rstan::check_hmc_diagnostics(m_noimpute_svp_noimpute_log_copy_16S_per_ml_sputum_high_count_brms$fit)
...
m_noimpute_svp_noimpute_log_copy_16S_per_ml_sputum_high_count_brms %>%
  ...
  as_tibble(rownames = "param") %>%
  mutate_if(is.numeric, list("OR" = ~ exp(.x))) %>%
  identity() -> m_noimpute_svp_noimpute_log_copy_16S_per_ml_sputum_high_count_param_est
m_noimpute_svp_noimpute_log_copy_16S_per_ml_sputum_high_count_param_est
}
...
#' mixed model plot
if(run_plots) {
m_noimpute_svp_noimpute_log_copy_16S_per_ml_sputum_high_count_brms$data %>%
  as_tibble() %>%
  expand(log_copy_16S_per_ml_sputum_high_count = modelr::seq_range(log_copy_16S_per_ml_sputum_high_count, n = 100), subject_id = unique(subject_id)) %>%
  tidybayes::add_fitted_draws(newdata = ., model = m_noimpute_svp_noimpute_log_copy_16S_per_ml_sputum_high_count_brms) %>%
  ungroup() %>%
  ggplot(data = ., aes(x = log_copy_16S_per_ml_sputum_high_count, y = .value)) +
  tidybayes::stat_lineribbon(.width = c(0.5, 0.8, 0.95), alpha = 0.7, color = colorspace::sequential_hcl(5, palette = "Blues 3")[1]) +
  colorspace::scale_fill_discrete_sequential(palette = "Blues 3", nmax = 5, order = 2:4) +
...
d_vap %>%
  brm(suspected_vap_plus_abx_and_pathogen_and_signs ~ log_copy_16S_per_ml_sputum_high_count + 0 + (1|subject_id),
      ...
      control = list("adapt_delta" = 0.99, max_treedepth = 16),
      backend = "cmdstanr",
      seed = 16) -> m_noimpute_svaps_log_copy_16S_per_ml_sputum_high_count_brms
m_noimpute_svaps_log_copy_16S_per_ml_sputum_high_count_brms

rstan::check_hmc_diagnostics(m_noimpute_svaps_log_copy_16S_per_ml_sputum_high_count_brms$fit)
...
m_noimpute_svaps_log_copy_16S_per_ml_sputum_high_count_brms %>%
  ...
  as_tibble(rownames = "param") %>%
  mutate_if(is.numeric, list("OR" = ~ exp(.x))) %>%
  identity() -> m_noimpute_svaps_log_copy_16S_per_ml_sputum_high_count_param_est
m_noimpute_svaps_log_copy_16S_per_ml_sputum_high_count_param_est
}
...
#' mixed model plot
if(run_plots) {
m_noimpute_svaps_log_copy_16S_per_ml_sputum_high_count_brms$data %>%
  as_tibble() %>%
  expand(log_copy_16S_per_ml_sputum_high_count = modelr::seq_range(log_copy_16S_per_ml_sputum_high_count, n = 100), subject_id = unique(subject_id)) %>%
  tidybayes::add_fitted_draws(newdata = ., model = m_noimpute_svaps_log_copy_16S_per_ml_sputum_high_count_brms) %>%
  ungroup() %>%
  ggplot(data = ., aes(x = log_copy_16S_per_ml_sputum_high_count, y = .value)) +
  tidybayes::stat_lineribbon(.width = c(0.5, 0.8, 0.95), alpha = 0.7, color = colorspace::sequential_hcl(5, palette = "Blues 3")[1]) +
  colorspace::scale_fill_discrete_sequential(palette = "Blues 3", nmax = 5, order = 2:4) +
...
dat %>%
  ...
  mutate(max_read_prop_noimpute_greater_than_0.5 = max_read_prop > 0.5) %>%
  group_by(subject_id) %>%
  arrange(subject_day) %>%
  mutate(max_read_prop_noimpute_improved = max_read_prop_noimpute_greater_than_0.5 == FALSE & lag(max_read_prop_noimpute_greater_than_0.5, 1) == TRUE) %>%
  replace_na(replace = list("max_read_prop_noimpute_improved" = FALSE)) %>%
  mutate(max_read_prop_noimpute_flip_noimpute_count = as.character(cumsum(max_read_prop_noimpute_improved)),
  ) %>%
  tidyr::fill(max_read_prop_noimpute_flip_noimpute_count, .direction = "down") %>% #arrange(desc(max_read_prop_noimpute_flip_noimpute_count)) %>%
  group_by(subject_id, max_read_prop_noimpute_flip_noimpute_count) %>%
  arrange(subject_day) %>%
  mutate(max_read_prop_noimpute_high_count = cumsum(max_read_prop_noimpute_greater_than_0.5)) %>%
  ungroup() %>% #qplot(data = ., x = max_read_prop_noimpute_high_count)
  distinct() %>%
  filter(complete.cases(.)) %>%
  identity() -> d_vap
...
#' run mixed effects model with data
d_vap %>%
  brm(suspected_vap ~ max_read_prop_noimpute_high_count + 0 + (1|subject_id),
      data = .,
      family = "bernoulli",
      iter = 2000,
      warmup = 1000,
      ...
      control = list("adapt_delta" = 0.99, max_treedepth = 16),
      backend = "cmdstanr",
      seed = 16) -> m_noimpute_sv_max_read_prop_noimpute_high_count_brms
m_noimpute_sv_max_read_prop_noimpute_high_count_brms

rstan::check_hmc_diagnostics(m_noimpute_sv_max_read_prop_noimpute_high_count_brms$fit)
...
m_noimpute_sv_max_read_prop_noimpute_high_count_brms %>%
  ...
  as_tibble(rownames = "param") %>%
  mutate_if(is.numeric, list("OR" = ~ exp(.x))) %>%
  identity() -> m_noimpute_sv_max_read_prop_noimpute_high_count_param_est
m_noimpute_sv_max_read_prop_noimpute_high_count_param_est
}
...
#' mixed model plot
if(run_plots) {
m_noimpute_sv_max_read_prop_noimpute_high_count_brms$data %>%
  as_tibble() %>%
  expand(max_read_prop_noimpute_high_count = modelr::seq_range(max_read_prop_noimpute_high_count, n = 100), subject_id = unique(subject_id)) %>%
  tidybayes::add_fitted_draws(newdata = ., model = m_noimpute_sv_max_read_prop_noimpute_high_count_brms) %>%
  ungroup() %>%
  ggplot(data = ., aes(x = max_read_prop_noimpute_high_count, y = .value)) +
  tidybayes::stat_lineribbon(.width = c(0.5, 0.8, 0.95), alpha = 0.7, color = colorspace::sequential_hcl(5, palette = "Blues 3")[1]) +
  colorspace::scale_fill_discrete_sequential(palette = "Blues 3", nmax = 5, order = 2:4) +
  facet_wrap(~ subject_id)
}
...
#' run mixed effects model with data
d_vap %>%
  brm(suspected_vap_plus_signs_and_pathogen ~ max_read_prop_noimpute_high_count + 0 + (1|subject_id),
      data = .,
      family = "bernoulli",
      iter = 2000,
      warmup = 1000,
      ...
      control = list("adapt_delta" = 0.99, max_treedepth = 16),
      backend = "cmdstanr",
      seed = 16) -> m_noimpute_svp_noimpute_max_read_prop_noimpute_high_count_brms
m_noimpute_svp_noimpute_max_read_prop_noimpute_high_count_brms

rstan::check_hmc_diagnostics(m_noimpute_svp_noimpute_max_read_prop_noimpute_high_count_brms$fit)
...
m_noimpute_svp_noimpute_max_read_prop_noimpute_high_count_brms %>%
  ...
  as_tibble(rownames = "param") %>%
  mutate_if(is.numeric, list("OR" = ~ exp(.x))) %>%
  identity() -> m_noimpute_svp_noimpute_max_read_prop_noimpute_high_count_param_est
m_noimpute_svp_noimpute_max_read_prop_noimpute_high_count_param_est
}
...
#' mixed model plot
if(run_plots) {
m_noimpute_svp_noimpute_max_read_prop_noimpute_high_count_brms$data %>%
  as_tibble() %>%
  expand(max_read_prop_noimpute_high_count = modelr::seq_range(max_read_prop_noimpute_high_count, n = 100), subject_id = unique(subject_id)) %>%
  tidybayes::add_fitted_draws(newdata = ., model = m_noimpute_svp_noimpute_max_read_prop_noimpute_high_count_brms) %>%
  ungroup() %>%
  ggplot(data = ., aes(x = max_read_prop_noimpute_high_count, y = .value)) +
  tidybayes::stat_lineribbon(.width = c(0.5, 0.8, 0.95), alpha = 0.7, color = colorspace::sequential_hcl(5, palette = "Blues 3")[1]) +
  colorspace::scale_fill_discrete_sequential(palette = "Blues 3", nmax = 5, order = 2:4) +
...
#' run mixed effects model with data
d_vap %>%
  brm(suspected_vap_plus_abx_and_pathogen_and_signs ~ max_read_prop_noimpute_high_count + 0 + (1|subject_id),
      data = .,
      family = "bernoulli",
      iter = 2000,
      warmup = 1000,
      ...
      control = list("adapt_delta" = 0.99, max_treedepth = 16),
      backend = "cmdstanr",
      seed = 16) -> m_noimpute_svaps_max_read_prop_noimpute_high_count_brms
m_noimpute_svaps_max_read_prop_noimpute_high_count_brms

rstan::check_hmc_diagnostics(m_noimpute_svaps_max_read_prop_noimpute_high_count_brms$fit)
...
m_noimpute_svaps_max_read_prop_noimpute_high_count_brms %>%
  ...
  as_tibble(rownames = "param") %>%
  mutate_if(is.numeric, list("OR" = ~ exp(.x))) %>%
  identity() -> m_noimpute_svaps_max_read_prop_noimpute_high_count_param_est
m_noimpute_svaps_max_read_prop_noimpute_high_count_param_est
}
...
#' mixed model plot
if(run_plots) {
m_noimpute_svaps_max_read_prop_noimpute_high_count_brms$data %>%
  as_tibble() %>%
  expand(max_read_prop_noimpute_high_count = modelr::seq_range(max_read_prop_noimpute_high_count, n = 100), subject_id = unique(subject_id)) %>%
  tidybayes::add_fitted_draws(newdata = ., model = m_noimpute_svaps_max_read_prop_noimpute_high_count_brms) %>%
  ungroup() %>%
  ggplot(data = ., aes(x = max_read_prop_noimpute_high_count, y = .value)) +
  tidybayes::stat_lineribbon(.width = c(0.5, 0.8, 0.95), alpha = 0.7, color = colorspace::sequential_hcl(5, palette = "Blues 3")[1]) +
  colorspace::scale_fill_discrete_sequential(palette = "Blues 3", nmax = 5, order = 2:4) +
...
tibble(value = c("m_noimpute_sv_shannon_low_count_param_est",
                 "m_noimpute_svp_noimpute_shannon_low_count_param_est",
                 "m_noimpute_svaps_shannon_low_count_param_est",
                 "m_noimpute_sv_max_read_prop_noimpute_high_count_param_est",
                 "m_noimpute_svp_noimpute_max_read_prop_noimpute_high_count_param_est",
  ...
  identity() -> m_noimpute_mixed_longitudinal_combined_parameter_estimates
m_noimpute_mixed_longitudinal_combined_parameter_estimates

if(save_models) {
m_noimpute_mixed_longitudinal_combined_parameter_estimates %>%
  write_csv(file = "./models/longitudinal/m_noimpute_mixed_longitudinal_combined_parameter_estimates.csv")
}
...
if(save_models) {
  tibble(value = c("m_noimpute_sv_shannon_low_count_brms",
                   "m_noimpute_svp_noimpute_shannon_low_count_brms",
                   "m_noimpute_svaps_shannon_low_count_brms",
                   "m_noimpute_sv_max_read_prop_noimpute_high_count_brms",
                   "m_noimpute_svp_noimpute_max_read_prop_noimpute_high_count_brms",
...
tibble(model_files = list.files(path = "./models/longitudinal/", pattern = "^m_noimpute_.*_brms\\.rds\\.gz", full.names = TRUE),
       model_names = stringr::str_remove(list.files(path = "./models/longitudinal/", pattern = "^m_noimpute_.*_brms\\.rds\\.gz", full.names = FALSE), pattern = "\\.rds\\.gz"),
) %>%
...
tibble(value = c("m_noimpute_sv_shannon_low_count_brms",
  ...
  mutate(param = gsub("m_noimpute_svp_noimpute_","b_",gsub("_brms","",value))) %>%
...
tibble(value = c("m_noimpute_sv_shannon_low_count_brms",
  ...
  filter(grepl("m_noimpute_svp_noimpute_",value)) %>%
  #
  ggplot(data = ., aes(y = parameter, x = posterior, fill = stat(x > 0))) +
  tidybayes::stat_halfeye(alpha = 0.8) +
  geom_vline(xintercept = 0, linetype = "dashed") +
...
tibble(value = c("m_noimpute_sv_shannon_low_count_brms",
  ...
  theme(axis.text.x = ggtext::element_markdown(color = "black"),
        ...
        legend.position = c(0.86,0.58)) +
  labs(x = "Odds Ratio of VA-LRTI per Day<br>of Persistent Microbiome Disruption",
       y = "",
       color = "Posterior<br>Credible<br>Interval") -> p_noimpute_persistent_mdi
p_noimpute_persistent_mdi



# p_noimpute_persistent_mdi %>%
#   ggsave(filename = "./figs/p_noimpute_persistent_mdi.pdf", height = 4, width = 5, units = "in")
# p_noimpute_persistent_mdi %>%
#   ggsave(filename = "./figs/p_noimpute_persistent_mdi.svg", height = 4, width = 5, units = "in")
# p_noimpute_persistent_mdi %>%
#   ggsave(filename = "./figs/p_noimpute_persistent_mdi.png", height = 4, width = 5, units = "in", dpi = 600)
...
tibble(value = c("m_noimpute_sv_shannon_low_count_brms",
  ...
  mutate(model = map(.x = value, .f = ~ get(.x))) %>%
  mutate(fit = map(.x = model, .f = ~ .x$fit)) %>%
  mutate(param = gsub("m_","b_",gsub("_brms","",gsub("sv_|svp_noimpute_|svap_|svaps_","",value)))) %>%
  mutate(posterior = map(.x = fit, .f = ~ posterior::as_draws_df(.x) %>% as_tibble() %>% select(contains("b_")) %>% pull())) %>%
  mutate(outcome_definition = stringr::str_extract(string = value, pattern = "sv_|svp_noimpute_|svap_|svaps_"),
         outcome_definition = case_when(outcome_definition == "sv_" ~ outcome_names[1],
                                        outcome_definition == "svp_noimpute_" ~ outcome_names[2],
  ...
  labs(x = "Odds Ratio of VA-LRTI per Day of Persistent Microbiome Disruption",
       y = "") -> p_noimpute_compare_outcome_definitions_persistence
p_noimpute_compare_outcome_definitions_persistence
